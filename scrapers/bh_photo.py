...
def scrape_bh_photo(session, retailer):
    ...
    """
    url = "https://www.bhphotovideo.com/c/search?q=rtx%205090"
    ...
    try:
        ...
        product_items = soup.select('.productBox')
        
        if not product_items:
            logger.warning("No product items found on B&H Photo")
            return results
        
        for item in product_items[:10]:  # Limit to top 10 results
            try:
                # Extract product details
                title_elem = item.select_one('.productNameText')
                if not title_elem:
                    continue
                ...
                # Extract product URL
                url_elem = item.select_one('a.productNameText')
                product_url = "https://www.bhphotovideo.com" + url_elem['href'] if url_elem else None
                if not product_url:
                    continue
                
                # Extract manufacturer
                manufacturer_elem = item.select_one('.productMfgText')
                manufacturer = manufacturer_elem.text.strip() if manufacturer_elem else "Unknown"
                ...
                if not product:
                    ...
                    description_elem = product_soup.select_one('.overviewTab')
                    description = description_elem.text.strip() if description_elem else "No description available"
                    
                    specs_elem = product_soup.select_one('#specsContent')
                ...
                in_stock = "Add to Cart" in item.text
        ...
        logger.error(f"Error scraping B&H Photo: {e}")
        return results